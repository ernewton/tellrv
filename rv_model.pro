PRO RV_WRAPPER, order, atrans, data, hdr, $
	data_tc, std, std_tc, shdr, $
	plorder=plorder, trange=trange, wrange=wrange, $
	shft=shft, s_shft=s_shft, rv=rv, $
	showplot=showplot
	...
	ENDCASE
	oversamp=6.d
	maxshft=0.0015/pixscale*oversamp
	...
	IF NOT KEYWORD_SET(norv) THEN BEGIN
		...
		IF NOT KEYWORD_SET(s_shft) THEN BEGIN
			...
			CASE order OF
				...
				ELSE: message, 'Order is', order, 'This is an invalid order'
			ENDCASE
			maxshft=0.0015/s_pixscale*oversamp
	
			; get wavelength calibration by modeling the data
		...
		RV, data_tc_new, std_tc_new, wrange=wrange, pixscale=pixscale, showplot=1, rv0=rv0

		rv=DO_BARY(rv0, hdr, shdr)
...
; 	wait, 1
	...
	IF KEYWORD_SET(showplot) THEN BEGIN
		...
		oplot, [trange[0], trange[0]], [-20,50], co=4, linestyle=2
		oplot, [trange[1], trange[1]], [-20,50], co=4, linestyle=2
...
PRO RV, data, std, pixscale=pixscale, wrange=wrange, showplot=showplot, rv0=rv0
	...
	; remove continuum
	flat_obj=FLATTEN(int_obj, /showplot)
	flat_std=FLATTEN(int_std, /showplot)
	...
	ENDIF
print, wl_vector[0], wl_vector[1]-wl_vector[0]
print, mean(flat_std), mean(flat_obj)
	corr_range=20
	...
	IF KEYWORD_SET(showplot) THEN BEGIN
		...
		plot, data[*,0], data[*,1]/MEAN(data[WHERE(FINITE(data[*,1])),1]), xrange=wrange, /xsty
		oplot, std[*,0], std[*,1]/MEAN(std[WHERE(FINITE(std[*,1])),1])-0.4
		oplot, data[*,0]-offset*data[*,0], data[*,1]/MEAN(data[WHERE(FINITE(data[*,1])),1])-0.2, co=3
...
FUNCTION FLATTEN, int, order, showplot=showplot
...
FUNCTION DO_BARY, rv0, hdr, shdr, rv_std=rv_std
